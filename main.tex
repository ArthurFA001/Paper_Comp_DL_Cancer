...
Dada la naturaleza crítica de la clasificación de tipos de cáncer y el desbalance de clases inherente al conjunto de datos, la evaluación del rendimiento no se limitó a la precisión global. Por consiguiente, se adoptó un enfoque multidimensional alineado con los estándares de la literatura en aprendizaje profundo clínico \cite{chicco2020advantages}, priorizando métricas que cuantifiquen la robustez de las predicciones en espacios de alta dimensionalidad.

\textit{Exactitud}

Se calculó la exactitud como la proporcion global de predicciones correctas. si bien es una metrica estandar para monitorear la convergencia, su interpretacion definitiva se debe realizar junto con otras métricas. En el contexto de aprendizaje profundo con datos desbalanceados, se ha demostrado que esta metrica puede ofrecer una visión incompleta del rendimiento del modelo \cite{johnson2019survey}.

\begin{equation*}
    \text{Accuracy} = \frac{TP + TN}{TP + TN + FP + FN}
\end{equation*}

\textit{Precisión, Sensibilidad y F1-Score}
